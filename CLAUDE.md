...
- **Telefe is embedded directly in the Tauri webview**, `targetKind: "url"`, `focusVideo: true`, pointing at `https://www.mitelefe.com/telefe-en-vivo`. `focusVideo` (a per-tile option, decided 2026-09-13) opens the page's main `<video>` full screen with sound and covers the rest of the ad-heavy page with a black backdrop, so the end user sees only live TV. It injects a resilient script (backdrop + lift the largest video, re-applied on a timer and DOM changes) rather than scraping a stream URL, which would be tokenised and break; web tiles set it from a switch in Admin, and it needs the WebView2 `--autoplay-policy=no-user-gesture-required` arg to start unmuted. Confirmed: this page serves its live stream via JW Player over HLS with standard pre-roll ads — no Widevine/VMP requirement, unlike Netflix — so it plays natively inside WebView2. If Telefe ever changes their player setup, this is the one integration point to re-validate.
