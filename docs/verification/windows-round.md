...
| 4a''' | Admin → Tiles → Add tile → "Un juego" adds Solitario; tap it on Home | Spider Solitaire opens offline (no internet needed), cards are large, tapping a card then a column moves it, "Repartir" deals, "Volver al inicio" returns |
